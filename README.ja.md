...
Tauri 2 ベースのデスクトップスクリーンショット翻訳ツールです。画面の領域を選択 → ローカル OCR でテキスト抽出 → AI 翻訳 → 右側パネルに結果表示。ポップアップや作業中断はありません。クイックテキスト入力に対応しており、ショートカットと入力テキストのマッピングを設定し、ショートカットキーでフォーカス中の入力欄に自動入力できます。
...
<p align="center">
  <img src="docs/assets/readme-desktop.png" alt="デスクトップとタスクバー" style="max-width: 30%; border-radius: 8px;" />
  <br/>
  <em>デスクトップショートカットとシステムモニター</em>
</p>

<p align="center">
  <img src="docs/assets/readme-tray-menu.png" alt="トレイメニュー" style="max-width: 30%; border-radius: 8px;" />
  <br/>
  <em>トレイ右クリックメニュー</em>
</p>
...
- **クイックテキスト入力** — ショートカットと入力テキストのマッピングを設定、ショートカットキーでフォーカス中の入力欄に自動入力、内蔵テンプレート対応（Git ワークツリーのプロンプトなど）
- **ローカル OCR** — Tesseract オフラインエンジン内蔵、中国語 / 英語 / 日本語対応、ソース言語の設定または自動検出が可能
- **AI 翻訳** — OpenAI 互換 / Anthropic / Gemini API 対応、モデルとキーは自己管理
...
- **多言語 UI** — 簡体字中国語 / English インターフェース、中国語 / 英語 / 日本語 / 韓国語 / フランス語 / ドイツ語 / スペイン語 / ロシア語への翻訳に対応、システム言語自動検出
- **ショートカット衝突検出** — ショートカットが他のアプリケーションに使用されている場合、自動的に検出して警告
...
- **API プロバイダー**: OpenAI / Anthropic / Gemini
- **API アドレス**: 各プロバイダーの API エンドポイント
- **API キー**: OS 認証情報マネージャーに安全保存、ディスクに書き込まれません
- **モデル名**: 例: `gpt-4o`、`claude-3-5-sonnet-20241022`、`gemini-2.5-flash`
...
システムトレイを右クリック → **クイックテキスト入力**、ショートカットと入力テキストのマッピングを設定:

- 「エントリ追加」をクリックして新しいショートカットマッピングを作成
- ショートカットキーと対応する入力テキストを設定
- 内蔵テンプレート（Git ワークツリーのプロンプトなど）による高速設定に対応
- 入力欄にフォーカスがある状態でショートカットキーを押すと自動入力
...
| UI | Ant Design (ダークテーマ) |
| バックエンド | Rust (2024 edition) |
| 状態管理 | Zustand 5 |
| ルーティング | react-router-dom 6 |
| 国際化 | react-i18next + i18next 24 |
| キャプチャ | xcap |
| OCR | Tesseract CLI (オフライン) |
| 翻訳 | reqwest → OpenAI 互換 / Anthropic / Gemini API |
...
cd SnapTranslate
pnpm install
pnpm run tauri dev    # 開発モード (HMR)
pnpm run tauri build  # プロダクションビルド
```
