...
Tauri 2 기반 데스크톱 스크린샷 번역 도구입니다. 화면 영역을 선택 → 로컬 OCR로 텍스트 추출 → AI 번역 → 우측 패널에 결과 표시. 팝업 없이 작업 흐름을 방해하지 않습니다.
...
- **AI 번역** — OpenAI 호환 API, 모델과 키 직접 준비
...
- **이중 언어 UI** — 간체 중국어 / English, 시스템 언어 자동 감지
...
| 프론트엔드 | Vue 3.5 + TypeScript + Vite 6 |
| UI | Naive UI (다크 테마) |
