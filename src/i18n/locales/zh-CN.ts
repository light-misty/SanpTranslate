export default {
  ...
  quickFill: {
    ...
    // 内置模板：创建工作树提示词
    templateGitWorktreeTitle: '创建工作树提示词',
    templateGitWorktreeText: [
      '请为当前 Git 仓库创建一个新的工作树（Worktree）：',
      '- 所有文件修改、git 操作都必须在切换后的新目录内完成。',
      '- 本任务允许并要求你进行提交推送，进行分阶段提交，每完成一个功能提交一次，再继续执行接下来的任务，每次提交都要推送，禁止一次性提交全部代码，禁止一次性开发完所有代码后分阶段提交，禁止推送到主分支。',
      '- 只准用 git commit 完成提交，禁止直接调用 git commit-tree、git hash-object -t commit、git mktree 这类底层 plumbing 命令来构造提交——除非你明确知道自己在做什么，且这种情况下必须显式传 -p <父提交> 指明父亲，并当场用 git cat-file -p <新提交> 核对对象里包含 parent 行。',
      '- 如果推送功能存在问题，那么只提交不推送。',
      '- 执行任何 git add / git commit 必做自检，在提交前对更改的文件进行验证，确保没有遗漏。',
      '- 分支名和路径名请根据下面的任务描述生成。',
      '- 命名规则：',
      ...
      '',
      '创建完成后，发布该 Branch，并切换至该工作树目录，然后执行以下任务：',
      '',
    ].join('\n'),
    // 内置模板：提交和推送指令
    templateCommitPushTitle: '提交和推送指令',
    templateCommitPushText: '本任务允许并要求你进行提交推送，进行分阶段提交，每完成一个功能提交一次，再继续执行接下来的任务，每次提交都要推送，禁止一次性提交全部代码，禁止一次性开发完所有代码后分阶段提交，禁止推送到主分支。 执行任何 git add / git commit 必做自检，在提交前对更改的文件进行验证，确保没有遗漏。',
  },
}
