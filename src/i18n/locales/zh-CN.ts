export default {
  // 通用
  common: {
    confirm: '确认',
    cancel: '取消',
    delete: '删除',
    copy: '复制',
    close: '关闭',
    loading: '加载中...',
    error: '操作失败',
  },

  // 控制栏
  controlBar: {
    translate: '翻译',
    retranslate: '重新翻译',
    translating: '翻译中...',
    copyTranslation: '复制译文',
    copyOriginal: '复制原文',
    recognizing: '识别中...',
    showOriginal: '显示原文',
    showTranslation: '显示译文',
    cacheHit: '已从历史缓存获取',
  },

  // 贴图窗口
  pin: {
    title: 'SnapTranslate - 贴图',
  },

  // 设置页面
  settings: {
    title: '设置',
    // 导航栏
    navGeneral: '通用',
    navApi: 'API',
    navTranslate: '翻译',
    navShortcut: '快捷键',
    navUpdate: '更新',
    navAbout: '关于',
    languageConfig: '界面语言',
    generalConfig: '通用设置',
    language: '语言',
    languageAuto: '跟随系统',
    languageZhCN: '简体中文',
    languageEnUS: 'English',
    apiConfig: 'API 配置',
    apiProvider: 'API 提供商',
    providerOpenai: 'OpenAI',
    providerAnthropic: 'Anthropic',
    providerGemini: 'Gemini',
    apiUrlPlaceholderOpenai: 'https://api.openai.com',
    apiUrlPlaceholderAnthropic: 'https://api.anthropic.com',
    apiUrlPlaceholderGemini: 'https://generativelanguage.googleapis.com',
    modelPlaceholderOpenai: 'gpt-4o',
    modelPlaceholderAnthropic: 'claude-3-5-sonnet-20241022',
    modelPlaceholderGemini: 'gemini-2.5-flash',
    apiBaseUrl: 'API 地址',
    apiKey: 'API 密钥',
    apiKeyPlaceholder: '请输入 API 密钥',
    model: '模型',
    translateConfig: '翻译配置',
    targetLanguage: '目标语言',
    ocrLanguage: '源语言',
    ocrLanguageAuto: '自动检测',
    shortcutConfig: '快捷键配置',
    captureShortcut: '截图快捷键',
    pinClipboardShortcut: '剪贴板贴图',
    textTranslateShortcut: '文本翻译',
    pressShortcut: '请按下快捷键...',
    clickToSet: '点击设置快捷键',
    restoreDefaults: '恢复默认',
    shortcutsRestored: '快捷键已恢复默认',
    shortcutOccupied: '已被其他程序占用',
    testConnection: '测试连接',
    saveFailed: '保存失败',
    loadFailed: '加载配置失败',
    fillApiUrl: '请先填写 API 地址',
    fillModel: '请先填写模型名称',
    fillApiKey: '请先配置 API 密钥',
    configFilePath: '配置文件路径',
    logFilePath: '日志文件路径',
    openFolder: '打开',
    deleteApiKey: '删除密钥',
    confirmDeleteApiKey: '确定要删除已保存的 API 密钥吗？',
    apiKeyDeleted: 'API 密钥已删除',
    deleteApiKeyFailed: '删除 API 密钥失败',
    apiKeyStoredInKeyring: '密钥已安全保存在系统密钥环中',
    saveApiKey: '保存密钥',
    apiKeySaved: 'API 密钥已保存',
    autoStart: '开机自启动',
    autoStartEnabled: '已开启开机自启动',
    autoStartDisabled: '已关闭开机自启动',
    autoStartFailed: '设置开机自启动失败',
    autoStartDisabledInDev: '开发环境禁止使用开机自启动',
    // 更新设置
    updateConfig: '更新设置',
    currentVersion: '当前版本',
    autoUpdate: '自动更新',
    checkUpdate: '检查更新',
    checkUpdateBtn: '检查更新',
    updateDisabledInDev: '开发模式下不可用',
    checkingUpdate: '正在检查更新...',
    updateAvailable: '发现新版本 {version}{date}',
    alreadyLatest: '当前已是最新版本',
    checkUpdateFailed: '检查更新失败: {error}',
    downloadAndInstall: '下载并安装',
    downloadingUpdate: '正在下载更新...',
    downloadProgress: '下载进度',
    updateDownloaded: '更新下载完成',
    updateReady: '更新就绪',
    updateReadyContent: '新版本已下载完成，需要重启应用以完成安装。是否立即重启？',
    restartNow: '立即重启',
    restartLater: '稍后重启',
    downloadFailed: '下载更新失败: {error}',
    updateStatus: '更新状态',
    // 目标语言选项
    langZhCN: '中文简体',
    langZhTW: '中文繁体',
    langEn: '英语',
    langJa: '日语',
    langKo: '韩语',
    langFr: '法语',
    langDe: '德语',
    langEs: '西班牙语',
    langRu: '俄语',
  },

  // 历史记录
  history: {
    title: '历史记录',
    clearAll: '清空全部',
    empty: '暂无翻译历史',
    detail: '翻译详情',
    original: '原文',
    translation: '译文',
    confirmDelete: '确定要删除这条翻译记录吗？',
    confirmClearAll: '确定要清空所有翻译历史吗？此操作不可撤销。',
    deleted: '已删除',
    cleared: '已清空所有历史记录',
    deleteFailed: '删除失败',
    clearFailed: '清空失败',
    loadFailed: '加载历史记录失败',
    detailLoadFailed: '加载详情失败',
    copySuccess: '已复制到剪贴板',
    copyFailed: '复制失败',
  },

  // 文本翻译
  textTranslate: {
    title: '文本翻译',
    inputPlaceholder: 'Ctrl + Enter 快捷翻译',
    shortcutHint: '双击关闭面板',
    translate: '翻译',
    retranslate: '重新翻译',
    translating: '翻译中...',
    copyTranslation: '复制译文',
    targetLanguage: '目标语言',
  },

  // 快捷文本填充
  quickFill: {
    title: '快捷文本填充',
    description: '配置快捷键和对应的填充文本。当焦点在输入框时，按下快捷键即可自动填充文本。',
    shortcut: '快捷键',
    shortcutPlaceholder: '点击设置快捷键',
    text: '填充文本',
    textPlaceholder: '输入要填充的文本内容...',
    addEntry: '添加条目',
    empty: '暂无配置，点击下方按钮添加',
    duplicateShortcut: '条目 #{first} 与条目 #{second} 使用了相同的快捷键 {shortcut}，请更换',
    conflictWithMain: '快捷键 {shortcut} 与系统主快捷键冲突，请更换',
    saveFailed: '保存失败：{error}',
    templates: '内置模板',
    templateEnabled: '已启用',
    templateDisabled: '未启用',
    // 内置模板：创建工作树提示词
    templateGitWorktreeTitle: '创建工作树提示词',
    templateGitWorktreeText: [
      '请为当前 Git 仓库创建一个新的工作树（Worktree）：',
      '- 所有文件修改、git 操作都必须在切换后的新目录内完成。',
      '- 只准用 git commit 完成提交，禁止直接调用 git commit-tree、git hash-object -t commit、git mktree 这类底层 plumbing 命令来构造提交——除非你明确知道自己在做什么，且这种情况下必须显式传 -p <父提交> 指明父亲，并当场用 git cat-file -p <新提交> 核对对象里包含 parent 行。',
      '- 如果推送功能存在问题，那么只提交不推送。',
      '- 执行任何 git add / git commit 必做自检，在提交前对更改的文件进行验证，确保没有遗漏。',
      '- 分支名和路径名请根据下面的任务描述生成。',
      '- 命名规则：',
      '  * 全部使用小写英文单词，多个单词之间用连字符（-）连接。',
      '  * 前缀根据任务类型决定：新功能使用 `feat/`，修复问题使用 `fix/` 等。',
      '  * 路径位于仓库父级目录下，格式为 `../<当前仓库名>-<分支名中的后缀部分>`。',
      '  * 如果生成的分支名已存在，则自动添加数字后缀（如 `-2`）。',
      '',
      '创建完成后，发布该 Branch，并切换至该工作树目录，然后执行以下任务：',
      '',
    ].join('\n'),
    // 内置模板：提交和推送指令
    templateCommitPushTitle: '提交和推送指令',
    templateCommitPushText: '本任务允许并要求你进行提交推送，进行分阶段提交，每完成一个功能提交一次，再继续执行接下来的任务，每次提交都要推送，禁止一次性提交全部代码，禁止一次性开发完所有代码后分阶段提交。 执行任何 git add / git commit 必做自检，在提交前对更改的文件进行验证，确保没有遗漏。',
  },

  // 任务待办
  taskTodo: {
    title: '任务待办',
    addTask: '添加任务',
    titlePlaceholder: '输入任务标题...',
    descriptionPlaceholder: '输入任务描述（可选）...',
    titleRequired: '请输入任务标题',
    priority: '优先级',
    priorityLow: '低',
    priorityMedium: '中',
    priorityHigh: '高',
    changePriority: '更改优先级',
    doubleClickToEdit: '双击编辑',
    done: '已完成',
    completedAt: '完成于',
    clearCompleted: '清空已完成',
    noCompletedTasks: '没有已完成的任务',
    clearedCompleted: '已清空 {count} 条已完成任务',
    stats: '进行中: {active} | 已完成: {completed} | 总计: {total}',
    filterAll: '全部',
    filterActive: '进行中',
    filterCompleted: '已完成',
    emptyAll: '暂无任务，点击上方按钮添加',
    emptyActive: '暂无进行中的任务',
    emptyCompleted: '暂无已完成的任务',
  },
}
